...
import { cssVar } from "../js/utils.js";
...
export const PROJECTS = [
  ...
  },
  {
    ...
    "dependencies": [],
    "enables": [],
    "body_html": "<h2>Goals</h2>\n<ul>\n<li>Build standardized Model Context Protocol (MCP) interfaces and agentic workflows so OpenFF and OpenFE can be called reliably by AI agents</li>\n<li>(External goal) Partner with Genentech to generalize and harden their OpenFE agent workflow (<a href=\"https://github.com/Genentech/openfe_skill\">openfe_skill</a>) — which already matched or exceeded two leading commercial solutions on three proprietary internal targets — and develop additional community agentic workflows</li>\n<li>(External goal) Build agent guardrails that detect and prevent physically nonsensical calculation setups (e.g. a ligand not bound to the protein, an incorrect protonation state, or an implausible system configuration) before they waste GPU resources</li>\n<li>(External goal) Deliver the Alchemiscale infrastructure improvements these production agentic workflows need: letting agents read and manage execution state, handle restarts, adjust strategy, and manage compute resources</li>\n</ul>\n<h2>Benefits</h2>\n<p>OpenFF and OpenFE are already the leading open-source stack for physics-based binding affinity prediction: a benchmark by 15 pharmaceutical companies across more than 1,700 ligands and 95 target systems found OpenFE's accuracy approaching leading commercial solutions at no licensing cost, using OpenFF Sage — which itself outperforms all publicly available alternatives in blind industry benchmarking. Both are core dependencies across academic and industry computational drug discovery workflows worldwide, with OpenFF alone used by more than 500 downstream repositories. But that scientific maturity hasn't translated into ease of use: running a full ligand design campaign still requires navigating multi-step workflows, cluster job submission, error interpretation, and configuration decisions that non-experts consistently struggle with — a barrier that has nothing to do with whether the underlying science is sound.</p>\n<p>Genentech's OpenFE agent already shows this barrier is solvable: demonstrated at the 2026 OMSF workshop, it matched or beat two leading commercial tools on three proprietary internal drug discovery programs, unassisted, and is now open source. Generalizing and hardening that workflow — plus adding guardrails that catch invalid setups before they burn GPU time, and giving agents the execution-state control Alchemiscale currently lacks — turns a promising internal exemplar into infrastructure any researcher or agent can rely on. That keeps state-of-the-art binding free energy prediction available to teams without commercial-license budgets, not just those who can pay for it, and gives the wider community an extensible base into which new methods can be dropped and measured against real discovery projects.</p>"
  },
  {
  ...
  },
  {
    ...
    "dependencies": [],
    "enables": [],
    "body_html": "<h2>Goals</h2>\n<ul>\n<li>Interface OpenFF with Modelforge so MLIP and ML/MM (mixed MLIP + force field) systems can be set up and run automatically for complex biomolecular systems</li>\n<li>(External goal) Enable OpenFE to extend binding free energy workfows to utilize MLIPs.</li>\n</ul>\n<h2>Benefits</h2>\n<p>Physics-based free energy simulation is the pharmaceutical industry's state of the art for drug design, and its accuracy is capped by the underlying force field. MLIPs — neural-network potentials trained bottom-up on QM energies and forces — are emerging as a near-QM-accuracy alternative, with foundation models already extrapolating well to unseen chemistry at a fraction of QM cost. But that's only useful to our users if they can actually set MLIPs up and run them through OpenFF/OpenFE; right now that tooling doesn't exist.</p>\n<p>This project puts OpenFF's existing force-field-fitting infrastructure to work on that gap, rather than leaving MLIP adoption in drug design to fragmented, one-off efforts. Downstream, it gives Open Free Energy's existing user base a path to MLIP-accuracy binding free energy calculations, and gives the wider community trustworthy, drug-design-specific benchmarks and leaderboards to evaluate MLIP architectures against — filling a real gap, since existing community MLIP benchmarks (e.g. ML-PEG) target materials science, not drug design.</p>"
  },
  {
    "id": "nagl-replacement",
...
// Colors are read from the CSS custom properties in css/main.css (single
// source of truth), not hardcoded here.
export const CATEGORIES = {
  infrastructure: { label: "Infrastructure / Fitting Pipeline", color: cssVar("--cat-infrastructure") },
  domain:         { label: "Domain Expansion",                  color: cssVar("--cat-domain") },
  accuracy:       { label: "Accuracy Improvements",             color: cssVar("--cat-accuracy") },
  usability:      { label: "Usability / Community",             color: cssVar("--cat-usability") },
};
